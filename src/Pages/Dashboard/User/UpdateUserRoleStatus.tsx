...
const UpdateUserRoleStatus: React.FC = () => {
	...
	const [updateUserRoleStatus, { isLoading }] = useUpdateUserRoleStatusMutation();

	// Determine type automatically if URL contains keywords
	const updateType: "ROLE" | "STATUS" =
		type === "ROLE" ? "ROLE" : type === "STATUS" ? "STATUS" : location.pathname.includes("update-user-role") ? "ROLE" : "STATUS";
	...
	const handleUpdate = async () => {
		...
		try {
			...
			toast.success(`User ${updateType.toLowerCase()} updated to "${value}" successfully!`);
			setUserId("");
			setValue("");
		} catch (err: any) {
			handleApiError(err);
