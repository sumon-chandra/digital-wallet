...
const UpdateProfile = () => {
	...
	const [updateProfile, { isLoading: isUpdateLoading }] = useUpdateMyProfileMutation();
		...
		<div className="flex justify-center items-center min-h-screen p-4 sm:p-8 bg-white text-black dark:bg-black dark:text-white transition-colors duration-500">
			<Card className="w-full max-w-md shadow-xl rounded-xl border-2 border-gray-200 dark:border-gray-800">
				<CardHeader>
					...
					<CardDescription className="text-gray-500 dark:text-gray-400">Update your account information and password.</CardDescription>
				</CardHeader>
				<CardContent>
					<form onSubmit={onSubmit} className="space-y-6">
						...
						<div className="space-y-2 relative">
							...
							<Input id="password" type={passwordType} placeholder="New Password" value={password} onChange={(e) => setPassword(e.target.value)} />
							<Button type="button" variant="ghost" size="sm" className="absolute right-0 top-7 h-8 px-2" onClick={togglePasswordVisibility}>
								{passwordType === "password" ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
							</Button>
						</div>
						<Button
							type="submit"
