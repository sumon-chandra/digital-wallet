			...
			<Card className="w-full max-w-4xl bg-card/90 backdrop-blur-sm shadow-2xl border border-border/40 rounded-3xl overflow-hidden">
				<CardHeader className="flex flex-col items-center gap-6 p-10 pb-8 border-b border-border/40 bg-gradient-to-r from-primary/5 via-primary/10 to-primary/5">
					...
					<div className="flex flex-col items-center space-y-3 text-center">
						<CardTitle className="text-4xl font-bold bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text text-transparent">{name}</CardTitle>
						<div className="flex items-center gap-2 text-muted-foreground text-lg">
							<Mail className="h-5 w-5" />
							{email}
						</div>
						<Badge variant="secondary" className="mt-4 text-base py-2 px-4 rounded-full flex items-center gap-2">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								{formFields.map((field) => (
									<div key={field.name} className="relative">
										<label htmlFor={field.name} className="block text-sm font-medium text-muted-foreground mb-2 ml-1">
											{field.placeholder}
										</label>
										<Input
											id={field.name}
						...
						<>
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								{profileInfo.map((info) => (
									<div key={info.label} className={`p-5 bg-muted/20 rounded-2xl ${info.colSpan}`}>
                          ...
                          className: "h-4 w-4",
										...
										{info.badge ? (
											<div className="flex items-center gap-2">
												...
												{is_verified ? <CheckCircle className="h-5 w-5 text-green-500" /> : <XCircle className="h-5 w-5 text-red-500" />}
											</div>
										) : (
