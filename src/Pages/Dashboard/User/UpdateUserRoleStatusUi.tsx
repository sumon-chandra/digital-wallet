...
const UpdateUserRoleStatusUi: React.FC<UpdateUserRoleStatusUiProps> = ({ type, userId, setUserId, value, setValue, onSubmit, isLoading }) => {
	const options = type === "ROLE" ? ["AGENT", "USER"] : ["ACTIVE", "INACTIVE", "BLOCKED"];
				...
				<CardHeader className="px-2 pb-8">
					<div className="flex items-center justify-center mb-2">
						<div className="p-4 rounded-2xl bg-primary/10">
							{type === "ROLE" ? <User className="w-12 h-12 text-primary" /> : <Activity className="w-12 h-12 text-primary" />}
						</div>
					</div>
					<CardTitle className="text-3xl font-bold text-center text-gray-900 dark:text-gray-100">Update User {type}</CardTitle>
					<p className="text-lg text-center text-gray-600 dark:text-gray-400 mt-2">
						{type === "ROLE" ? "Change user permissions and access levels" : "Manage user account status and accessibility"}
					</p>
				</CardHeader>
				...
				<CardContent className="px-2 space-y-10">
					<div className="grid gap-6">
						<div className="space-y-3">
							<Label htmlFor="userId" className="text-xl font-semibold text-gray-800 dark:text-gray-200">
								User Identification
								<span className="text-sm font-normal">
									(Before making changes, please ensure you have copied the user ID from the{" "}
									<Link to="/admin/dashboard/all-users" className="text-primary hover:underline">
										All Users
									</Link>{" "}
									page.)
								</span>
						...
						<div className="space-y-3">
							<Label htmlFor="valueSelect" className="text-xl font-semibold text-gray-800 dark:text-gray-200">
								Select New {type}
							</Label>
							<Select value={value} onValueChange={setValue}>
								...
								<SelectContent className="rounded-xl">
									{options.map((opt) => (
										<SelectItem key={opt} value={opt} className="text-lg py-3 px-4 my-1 rounded-lg">
